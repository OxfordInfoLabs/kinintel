...
class FeedImportExporterTest extends TestBase {
    ...
    public function testCanGetAllExportableResources() {
        ...
        $this->feedService->returnValue("filterFeeds", [
            new FeedSummary("/test1", 1, ["input1"], "json", [], false, '', "query", 0, null, 55),
            new FeedSummary("/test2", 2, ["input2"], "json", [], false, '', "query", 0, null, 66),
            new FeedSummary("/test3", 3, ["input3"], "json", [], false, '', "query", 0, null, 77),
        ], [
    ...
    public function testCanCreateExportResourcesWithSubstitutedValues() {
        ...
        $this->assertEquals([
            new FeedSummary("/test1", -1, ["input1"], "json", [], false, '', "query", 0, null, -1),
            new FeedSummary("/test3", -3, ["input3"], "json", [], false, '', "query", 0, null, -2),
        ], $exported);
    ...
    public function testCanImportFeedsFromExport() {
        ...
        $this->assertTrue($this->feedService->methodWasCalled("saveFeed",[
            new FeedSummary("/test1", 33, ["input1"], "json", [], false, '', "query", 0, null, 55),
            "testProject",
            5
        ]));

        $this->assertTrue($this->feedService->methodWasCalled("saveFeed",[
            new FeedSummary("/test3", 44, ["input3"], "json", [], false, '', "query", 0, null),
            "testProject",
            5
        ]));
